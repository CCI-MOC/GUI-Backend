...
@task(name="sync_os_compute_nodes")
def sync_os_compute_nodes(atm_user_id, atm_project_id, os_compute_url, os_scoped_token):
        # when moving this over, get the project from the database and from that
        # get the os_project_id
        #
        # This is the start of syncing the compute nodes to instances.  This is only
        # included here as a proof of concept.
        response = requests.get(os_compute_url + '/servers',
                                headers={'x-auth-token': os_scoped_token})
        try:
            project_servers = json.loads(response.text)
        except KeyError:
            logger.info("cannot find Openstack project servers for atm_project_id: %s " % (str(atm_project_id)))
        celery_logger.info("end of sync_project")
...
def get_atm_project(os_project, group_id):
    """
    This gets the project from the database
    """
    try:
...
def sync_atm_with_openstack(identity_id):
    ...
    for os_project in os_project_list:
        logger.info("Syncing OS Project: %s:%s %s" % (str(os_project.domain_id),
                    str(os_project.name), str(os_project.id)))
        atm_project = get_atm_project(os_project, atm_group.id)
        ...
        sync_os_compute_nodes.delay(atm_user.id,
                                    ...
                                    scoped_token)
        # get volumnes
    logger.info("Remove atmosphere projects that are not in OpenStack")
    ...
    for atm_project in all_atm_projects:
        if atm_project.id not in active_atm_projects:
            atm_project.delete()
    set_active_os_project(os_project_list[0].id)
