...
class UUIDHyperlinkedIdentityField(serializers.HyperlinkedIdentityField):
    ...
    def get_url(self, obj, view_name, request, format):
        """
        Given an object, return the URL that hyperlinks to the object based on
        lookup_field. Raises a 'NoReverseMatch' without a 'lookup_field'.
        """
        if obj.pk is None:
            return None
...
class InstanceSourceHyperlinkedIdentityField(serializers.HyperlinkedIdentityField):
    ...
    def get_url(self, obj, view_name, request, format):
        """
        Given an object, return the URL that hyperlinks to the object based on lookup_field.
        Raises a 'NoReverseMatch' without a 'lookup_field'.
        """
